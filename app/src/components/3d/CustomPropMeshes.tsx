...
function CustomMesh({ id, position, rotation, dims, shape, selected, onSelect, onUpdatePosition, stackMode }: CustomMeshProps) {
  ...
  const geometry = useMemo(() => {
    if (shape === 'cylinder') { const radius = (dims.diameter || 50) / 2; return new THREE.CylinderGeometry(radius * MM_TO_UNIT, radius * MM_TO_UNIT, dims.h * MM_TO_UNIT, 16); }
  ...
  const material = useMemo(() => new THREE.MeshStandardMaterial({ color: '#CCCCCC', roughness: 0.7, metalness: 0.1, transparent: true, opacity: 0.6 }), []);
  const highlightMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#DDDDDD', roughness: 0.5, metalness: 0.15, emissive: '#4A9EFF', emissiveIntensity: 0.25, transparent: true, opacity: 0.75 }), []);
  ...
  return (
    <>
      {customProps.map((prop) => {
        ...
        return <CustomMesh key={prop.id} id={prop.id} position={[prop.position.x, prop.position.y, prop.position.z]} rotation={[prop.rotation.x, prop.rotation.y, prop.rotation.z]} dims={prop.dims} shape={prop.shape} selected={isSelected(prop.id)} onSelect={selectObject} onUpdatePosition={updateObjectPosition} stackMode={prop.stackMode} />;
      })}
    </>
  );
}
