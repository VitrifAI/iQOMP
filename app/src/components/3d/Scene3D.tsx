...
function CameraController() {
  ...
  // Snap the camera to a named shot angle (elevation/azimuth around the origin).
  // Driven independently of the preset effect; picking a preset clears shotAngle.
  useEffect(() => {
    if (!shotAngle) return;
    const r = 460 * MM_TO_UNIT;
    const el = (shotAngle.elevation * Math.PI) / 180;
    const az = (shotAngle.azimuth * Math.PI) / 180;
    const tx = 0, ty = 20 * MM_TO_UNIT, tz = 0;
    camera.position.set(
...
export function Scene3D() {
  ...
  const cropOverlayEnabled = useAppStore((s) => s.cropOverlayEnabled);
  ...
  return (
    ...
    >
      ...
      {cropOverlayEnabled && <CropOverlay />}
    </div>
  );
}
