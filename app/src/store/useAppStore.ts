...
interface AppState {
  ...
  // Stacking & labels
  stackEnabledIds: string[];
  stackLift: Record<string, number>;
  materialText: Record<string, string>;

  // Crop overlay
  cropOverlayEnabled: boolean;

  // Shot angle
  shotAngle: ShotAngle | null;
  ...
  toggleObjectStack: (id: string) => void;
  setStackLifts: (lifts: Record<string, number>) => void;
  setObjectMaterial: (id: string, text: string) => void;
  toggleCropOverlay: () => void;
  setShotAngle: (name: string, elevation: number, azimuth: number) => void;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  toggleProp: (propId) =>
    set((state) => {
      ...
      const dropSet = new Set(removedMeshIds);
      const filteredMaterial = Object.fromEntries(
        Object.entries(state.materialText).filter(([k]) => !dropSet.has(k))
      );
      const filteredLift = Object.fromEntries(
        Object.entries(state.stackLift).filter(([k]) => !dropSet.has(k))
      );
      return {
        activeProps: newProps,
        selectedObjectIds: state.selectedObjectIds.filter((id) => !dropSet.has(id)),
        lockedIds: state.lockedIds.filter((id) => !dropSet.has(id)),
        stackEnabledIds: state.stackEnabledIds.filter((id) => !dropSet.has(id)),
        materialText: filteredMaterial,
        stackLift: filteredLift,
      };
    }),
  ...
  setCameraPreset: (preset) =>
    set({ cameraPreset: preset, cameraMode: 'preset', shotAngle: null }),
  ...
  toggleObjectStack: (id) =>
    set((state) => {
      if (state.stackEnabledIds.includes(id)) {
        const { [id]: _drop, ...restLift } = state.stackLift;
        return {
          stackEnabledIds: state.stackEnabledIds.filter((x) => x !== id),
          stackLift: restLift,
        };
      }
      return { stackEnabledIds: [...state.stackEnabledIds, id] };
    }),

  setStackLifts: (lifts) => set({ stackLift: lifts }),

  setObjectMaterial: (id, text) =>
    set((state) => {
      const next = { ...state.materialText };
      if (text && text.trim().length > 0) next[id] = text;
      else delete next[id];
      return { materialText: next };
    }),

  toggleCropOverlay: () => set((state) => ({ cropOverlayEnabled: !state.cropOverlayEnabled })),

  setShotAngle: (name, elevation, azimuth) =>
    set({ shotAngle: { name, elevation, azimuth } }),
